...
const BODIES = {
  ...
  'codebase-map': '# Codebase Map\n\n## Entry Points\nsrc/server.js\n\n## Module Boundaries\ncheckout\n\n## Data Model\nCart\n\n## Cross-Cutting Concerns\nauth\n\n## Test Topology\ntest/\n\n## Change Hotspots\nsrc/checkout.js\n',
  glossary: '# Glossary\n\n- **Cart** → `CartAggregate`\n',
  'requirements-spec': '# Requirements\n\n## In Plain Terms\nWhen a shopper is logged out mid-checkout, we keep their cart so they can carry on after signing back in, instead of losing it.\n\n## Context\nPROJ-412.\n\n## In Scope\n1. Preserve cart.\n\n## Out of Scope\nGuest checkout.\n\n## Functional Requirements\nFR-1 Cart survives expiry.\n\n## Non-Functional Requirements\np95 < 300ms.\n\n## Data\nCart rows.\n\n## Dependencies\nNone.\n\n## Assumptions\nSessions are server-side.\n\n## Decisions Required\nNone outstanding.\n',
  ...
  'ux-lofi': '# Low-Fidelity\n\n## In Plain Terms\nThe shopper hits a sign-in screen, logs in, and lands back where they were with the cart still there. One new screen, one extra step.\n\n## User Flows\n1. Expired → sign-in → cart intact\n\n## Screen Inventory\n| ID | Screen |\n|---|---|\n| S1 | Sign-in |\n\n## Wireframes\n```\n+---+\n```\n\n## Open Questions\nNone.\n',
  'ux-midfi': '# Mid-Fidelity\n\n## In Plain Terms\nWe spell out what the shopper sees when the session has expired, when the page is loading, and when something errors — not just the normal case.\n\n## States\n| Screen | State |\n|---|---|\n| S1 | error |\n\n## Interaction Specification\nSubmit.\n\n## Content & Messaging\n"Your session expired."\n\n## Responsive Behaviour\n1 breakpoint.\n\n## Validation Rules\nRequired.\n',
  'ux-hifi': '# High-Fidelity\n\n## In Plain Terms\nThe finished look: a standard alert banner explaining the session expired, meeting our contrast and keyboard-access standards. No new custom components.\n\n## Design System Usage\n| Element | Component |\n|---|---|\n| Banner | Alert |\n\n## Visual Specification\ntokens only.\n\n## Accessibility\nContrast 4.6:1, keyboard path defined, live region on error.\n\n## Asset Manifest\nNone.\n\n## Implementation Notes\nUse Alert.\n',
  ...
  'architecture-spec': '# Architecture\n\n## In Plain Terms\nRight now the cart only lives in the browser session, so logging out loses it. We will save the cart on the server just before sending the shopper to sign-in, then restore it when they return. This adds one write to the checkout path (a small speed cost) and a new database table. The cart stays owned by the server, not the browser. Main risk: if that save silently fails the shopper still loses the cart, so we add an alert on it. Approving this commits us to the new table and the extra write; reversing it later means unpicking both.\n\n## Approach\nPersist cart before redirect.\n\n## Component Map\n| Component | Path |\n|---|---|\n| checkout | src/checkout.js |\n\n## Interfaces\nPOST /checkout\n\n## User Flow\n1. submit → 2. session expired → 3. persist cart → 4. redirect to sign-in → 5. resume\n\n## Frontend Design\nBanner component reused; cart state stays server-owned.\n\n## Data Design\ncarts table.\n\n## Sequence\n1. expire 2. persist 3. redirect\n\n## Security\nAuthZ on cart owner.\n\n## Observability\nMetric cart.preserved\n\n## Performance\np95 300ms.\n\n## Alternatives Considered\nClient storage — rejected, PCI.\n',
  ...
  'review-report': '# Code Review\n\n**Verdict**: approve\n\n## In Plain Terms\nThe code does what the design said and the tests cover it. No blocking problems found, so it is fine to move forward.\n\n## Summary\nMatches the design.\n\n## Blockers\nNone.\n\n## Findings\nNone.\n\n## Nits\nNone.\n\n## AC Coverage\n| AC | Implemented | Tested |\n|---|---|---|\n| AC-1 | yes | yes |\n\n## Deviations Reviewed\nNone.\n\n## What I Verified\nRead src/checkout.js; ran npm test.\n',
  ...
  'release-notes': '# Release Notes\n\n## In Plain Terms\nShipping the change that keeps a shopper cart through a session expiry. Verified by 41 passing tests. If it misbehaves, turn off the feature flag to roll back; the database change is additive and safe to leave.\n\n## Summary\nCart survives expiry.\n\n## Changes\nWP-1 (PROJ-412)\n\n## Verification\n41 tests pass.\n\n## Risk & rollback\nAdditive migration; disable the flag to roll back.\n\n## Documentation\ndocs/checkout.md updated.\n\n## Follow-ups\nLoad test at peak.\n',
...
for (let guard = 0; guard < 40; guard++) {
  ...
  if (open.length) {
    const g = open[0];
    gatesHit++;
    // HERMIT-18: the gate, and the message a caller sees, must carry both the
    // static "what approving means" line and the producing agent's own
    // `## In Plain Terms` briefing lifted out of the submitted artifact.
    assert.ok(g.plain && g.plain.length > 40, `gate on ${g.stageId} has no plain-language explanation`);
    const gStage = getStage(DEFAULT_PIPELINE, g.stageId);
    const expectBriefing = (gStage.outputs ?? []).some((o) =>
      (gStage.exitCriteria ?? []).some((c) => c.artifact === o && c.value === '## In Plain Terms')
    );
    if (expectBriefing) {
      assert.ok(
        Array.isArray(g.plainBriefing) && g.plainBriefing.length > 0,
        `gate on ${g.stageId} must carry the submitted \`## In Plain Terms\` briefing`
      );
    }
    const waiting = nextTask({ paths, run: loadRun(paths, run.id), registry: reg });
    assert.equal(waiting.state, 'awaiting_gate');
    assert.ok(
      waiting.message.includes('What approving means:') && waiting.message.includes(g.plain),
      `the awaiting_gate message for ${g.stageId} must spell out what approving means`
    );
    if (expectBriefing) {
      assert.ok(
        waiting.message.includes('In plain terms — what was submitted') &&
          waiting.message.includes(g.plainBriefing[0].text.split('\n')[0]),
        `the awaiting_gate message for ${g.stageId} must include the submitted briefing text`
      );
    }
    if (g.stageId === 'architecture') {
      assert.ok(
        waiting.message.includes('save the cart on the server'),
        'the architecture gate message must carry the architect\'s own plain-language account of the design'
      );
    }
